...
TypeScript inherits JavaScript's type system, where the primary number-related types are

- `boolean`:
  Either `true` or `false`.
- `number`:
  64-bit floating point (IEEE 754). For integer operations, safe range is `-(2^53-1)` to `2^53-1` (`Number.MAX_SAFE_INTEGER`).
- `bigint`:
  Arbitrary precision integer for values beyond the safe integer range.
...
Bitwise operators in JavaScript/TypeScript operate on 32-bit signed integers
*/
let num = ...
let n = ...
num &= n // AND
...
num >>= n // Arithmetic right shift 'n' bits from 'num' => Divide 'num' by 2^'n'. Fills left-side bits with MSB (sign-preserving)
...
2 ** 3 // Returns: 8
7 / 2 // Returns: 3.5
Math.trunc(7 / 2) // Returns: 3 => Quotient (integer division)
...
Random functions
*/
Math.floor(Math.random() * 5) // Returns: Random integer of range [0, 5)
Math.floor(Math.random() * (3 - 0 + 1)) + 0 // Returns: Random integer of range [0, 3]
Math.random() // Returns: Random number between [0.0, 1.0)
// For in-place array shuffle (Fisher-Yates)
