...
- Merge Sort
  ...
  function mergeSort(nums: number[]): void {
      ...
      */
      ...
      function merge(loNums: number[], hiNums: number[]): void {
          let i = 0, j = 0, k = 0
...
JavaScript/TypeScript uses TimSort
TC: O(NlogN)
SC: O(N) Average
*/
const nums: number[] = [3, 1, 6, 5]
nums.sort((a, b) => a - b) // State: [1, 3, 5, 6] => *In-place*. **IMPORTANT**: Always pass comparator for numbers, default sort is lexicographic
...
TypeScript does not have a built-in bisect module. Below is a simple implementation.
*/
...
const nums2: number[] = [0, 1, 2, 2, 3, 6]
const l = bisectLeft(nums2, 2) // Returns: 2 => Left-most place in the sorted array to insert the given element
const r = bisectRight(nums2, 2) // Returns: 4 => Right-most place in the sorted array to insert the given element
if (l === r) { /* It means value is not found */ }
if (l !== r) {
    // It means value is found
    // To find the actual index of the first and last occurrence of the 'val', it should be either 'l' or 'r - 1'
}

bisectLeft(nums2, 12, 1, 5) // Returns: 5 => Parameters allow range search. Index will return based on the scope of range
bisectRight(nums2, 12, 1, 5) // Returns: 5 => Parameters allow range search. Index will return based on the scope of range
```
- Almost all Binary Search questions involve using either
  - Arrays that is given either sorted or unsorted.
    - Not sorted:
    ...
    - Sorted:
      ...
      */
      function binarySearch(nums: number[], trg: number): number {
          let lo = 0
...
TypeScript/JavaScript does not have a built-in Red-Black tree.
However, there are 3rd-party libraries such as [bintrees](https://www.npmjs.com/package/bintrees) or [sorted-btree](https://www.npmjs.com/package/sorted-btree) that can be used in interviews.
