...
- Directed Graph:
  ...
  - Trie:
    ...
    class Trie {
        ...
        constructor() {
            this.root = { childs: new Map(), end: false }
        }

        insert(txt: string): void {
            let cur = this.root
            for (const ch of txt) {
                if (!cur.childs.has(ch)) {
                    cur.childs.set(ch, { childs: new Map(), end: false })
                }
        ...
        startWith(pfx: string): boolean {
            let cur = this.root
            for (const ch of pfx) {
                if (!cur.childs.has(ch)) return false
                cur = cur.childs.get(ch)!
            }
            return true
        }
        ...
        longestPrefix(): string {
            const pfx: string[] = []
            let cur = this.root
            while (cur.childs.size === 1) {
                const [ch, child] = [...cur.childs.entries()][0]
                if (!cur.end) {
                    pfx.push(ch)
                    cur = child
                } else {
